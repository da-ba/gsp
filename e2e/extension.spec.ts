...
test.describe("Flexible Command Parsing", () => {
  let testServer: { server: Server; port: number }
  ...
  test("command triggers picker when typed mid-sentence", async () => {
    const context = await chromium.launch({ headless: false })
    const page = await context.newPage()
    ...
    await textarea.fill("some text before /gsp")
    await page.waitForTimeout(500)
  ...
  test("picker stays open when typing query after command", async () => {
    const context = await chromium.launch({ headless: false })
    const page = await context.newPage()
    ...
    // Add more text (query) - picker should stay open
  ...
  test("picker closes only when command name is deleted", async () => {
    const context = await chromium.launch({ headless: false })
    const page = await context.newPage()
    ...
    // Delete the command (picker should close)
    await textarea.fill("/gs") // incomplete command
    await page.waitForTimeout(300)

    // Picker should close because "gs" is not a valid command
    await expect(picker).not.toBeVisible()

    await context.close()
  })

  test("picker closes on Escape even with query text", async () => {
    const context = await chromium.launch({ headless: false })
    const page = await context.newPage()
    ...
    // Type command with query
    await textarea.fill("/font some query")
    await page.waitForTimeout(500)
  ...
  test("command works after other text on same line", async () => {
    const context = await chromium.launch({ headless: false })
    const page = await context.newPage()
    ...
    // Type text then command then query
    await textarea.fill("prefix /emoji fire")
    await page.waitForTimeout(500)

    const picker = page.locator("#slashPalettePicker")
    await expect(picker).toBeVisible({ timeout: 3000 })

    // The query should be recognized and filtered
    // Press Enter to select
    await page.keyboard.press("Enter")
    await page.waitForTimeout(300)

    // The command replaces only the /emoji part, preserving text before it
    const textareaValue = await textarea.inputValue()
    // Should preserve the prefix text
    expect(textareaValue.startsWith("prefix ")).toBe(true)
    // Should have inserted some emoji after the prefix
    expect(textareaValue.length).toBeGreaterThan("prefix ".length)

    await context.close()
  })

  test("selection inserts content at command location", async () => {
    const context = await chromium.launch({ headless: false })
    const page = await context.newPage()
    ...
    // Should have inserted **bold** replacing the command
    const textareaValue = await textarea.inputValue()
...
test.describe("Mention Command", () => {
  ...
  test("/mention search filters users", async () => {
    ...
    expect(pickerContent?.includes("Matching") || pickerContent?.includes("No matching")).toBe(true)
